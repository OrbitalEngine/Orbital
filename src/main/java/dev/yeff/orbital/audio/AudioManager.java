...
public class AudioManager {
  @Getter private static final List<Music> musicStreams = new ArrayList<>();
  ...
  public static void pauseAudioClip(AudioClip audio) {
    if (audio.isPlaying()) PauseSound(audio.asRaylibSound());
    else throw new IllegalStateException("Audio clip is not playing, cannot pause.");
  }
  ...
  public static void pauseMusic(Music music) {
    if (music.isPlaying()) PauseMusicStream(music.asRaylibMusic());
    else throw new IllegalStateException("Audio clip is not playing, cannot pause.");
  }
